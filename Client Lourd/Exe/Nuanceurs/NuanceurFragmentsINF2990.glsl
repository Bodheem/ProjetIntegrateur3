...
void main()
{
	...
	vec4 textureColor = texture2D( laTexture, gl_TexCoord[0].st );
		if(etatNoeud == NOEUD_GHOST)
		textureColor *= vec4(1.0, 1.0, 1.0, 0.10);
	...
	if(colorShift == 2) colorMask = vec4(1.0,0.5,0.0,1.0);
	if(colorShift == 3) colorMask = vec4(0.5,1.0,0.5,0.0);
	...
	// **** application bougée à texturecolor plus haut. ****
	//if(etatNoeud == NOEUD_GHOST)
		//colorMask *= vec4(1.0, 1.0, 1.0, 0.10);
	...
	if(skybox == 1 && etatNoeud != NOEUD_GHOST)
	{

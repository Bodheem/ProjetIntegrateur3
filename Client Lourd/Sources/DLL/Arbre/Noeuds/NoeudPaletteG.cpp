					...
					&& parent->getEnfant(i)->estAffiche())
				{
					...
					while (j < boite.size() && contact == false)
					{
						if (type != "butoirg" && type != "butoird")
						{
							...
							{
								if ((type == "paletteg" || type == "paletted") /*&& parent->getEnfant(i)->getTransparent()*/)
								{
									// Dank memes
								}
								else
...
/// @brief Cette fonction active l'animation qui fait monter les palettes. On en
...
void NoeudPaletteG::activer()
{
	if (isBlocked_)
	{
		// Pour la désactivation temporaire des portails
		std::clock_t time = std::clock();
		double diff = (time - start_) / static_cast<double>(CLOCKS_PER_SEC);
		...
		if (diff > 10)
		{
			...
			{
				FacadeModele::obtenirInstance()->obtenirControleurSon()->jouerSon("palette");
				etatPalette_ = ACTIVE;
			}
...
/// @brief Cette fonction active l'animation qui fait revenir les palettes.
...
void NoeudPaletteG::traiterCollisions(aidecollision::DetailsCollision details, NoeudAbstrait* bille, float)
{
	...
	if (isCompetitif && (bille->getColorShift() != colorShift_ && isOnline))
		return;
	...
	// Effet sonore
	FacadeModele::obtenirInstance()->obtenirControleurSon()->jouerSon("palette2");
...
////////////////////////////////////////////////////////////////////////
/// @fn void NoeudPaletteG::setStartingDisable(std::clock_t starting)
/// @brief Cette fonction assigne le moment ou on commence a desactiver les palettes
/// @param[in] starting le temps
/// @return Aucun.
////////////////////////////////////////////////////////////////////////
void NoeudPaletteG::setStartingDisable(std::clock_t starting)
{
	start_ = starting;
	isBlocked_ = true;
}
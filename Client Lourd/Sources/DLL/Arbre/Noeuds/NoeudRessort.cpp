...
void NoeudRessort::compresser()
{
	...
	{
		FacadeModele::obtenirInstance()->obtenirControleurSon()->arreterSon("spring");
	//	double longueurOriginale_ = scale_.y *(boite_.coinMax.y - boite_.coinMin.y);
		...
		FacadeModele::obtenirInstance()->obtenirControleurSon()->jouerSon("springComp");
	}
...
{
	FacadeModele::obtenirInstance()->obtenirControleurSon()->arreterSon("springComp");
	if (etatRessort_ == AU_REPOS)
		return;
	...
	FacadeModele::obtenirInstance()->obtenirControleurSon()->jouerSon("spring");
}
...
void NoeudRessort::traiterCollisions(aidecollision::DetailsCollision details, NoeudAbstrait* bille, float)
{
	...
	if ((bille->getColorShift() != colorShift_) && isOnline)
		return;

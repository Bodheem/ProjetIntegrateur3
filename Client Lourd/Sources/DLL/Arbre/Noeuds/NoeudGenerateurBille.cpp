...
void NoeudGenerateurBille::animer(float temps)
{
	...
	else if (billesGenerer_ > 0)
	{
		...
		else
		{
			...
			billesGenerer_ = 0;
			FacadeModele::obtenirInstance()->obtenirGlobalesPartie()->finGeneration();
		}
		return;
	}
...
/// @brief Cette fonction effectue l'animation "shake" 
...
void NoeudGenerateurBille::shakeHead(float temps)
{
	...
	if (etatGenerateur_ == INITIAL) {
		...
		{ 
			controleurSon->jouerSon("generateur", "true");
			return;
		}
	...
	if (leSonJoue == false)
	{
		controleurSon->jouerSon("generateur");
		leSonJoue = true;	
	}
}

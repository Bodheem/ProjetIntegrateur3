...
void NoeudButoirG::traiterCollisions(aidecollision::DetailsCollision details, NoeudAbstrait* bille, float facteurRebond)
{
	...
	{
		facadeModele->obtenirGlobalesPartie()->collisionButoirTriangulaire(bille->getColorShift());
		compteurIllumination_ = 0;
		if (facadeModele->obtenirModeForceRebond())
			facteurRebond = FACTEUR_REBOND_BUTOIRE;
	}
	NoeudAbstrait::traiterCollisions(details, bille, facteurRebond);
	facadeModele->obtenirControleurSon()->jouerSon("butoir");
}
